...
from transformers import (
    AutoModelForSequenceClassification,  # ← 수정
    AutoTokenizer,
...
# 1. 데이터 로드
# ──────────────────────────────────────────
with open(r"C:\dev\TalkTrip\TripTalk_ai_model\dataset\classification_balanced.json", 'r', encoding='utf-8') as f:
...
# 5. 모델 (AutoModel로 수정)
# ──────────────────────────────────────────
model = AutoModelForSequenceClassification.from_pretrained(  # ← 수정
    "klue/roberta-base",
    num_labels=2,
...
# ──────────────────────────────────────────
class EarlyStopping:
    def __init__(self, patience=5, min_delta=0.001):  # ← patience 5로 늘림
...
for epoch in range(100):
    ...
    if val_acc > best_val_acc:
        ...
        model.save_pretrained("./model1_balence_best")
        tokenizer.save_pretrained("./model1_balence_best")
...
# 9. 테스트
# ──────────────────────────────────────────
model = AutoModelForSequenceClassification.from_pretrained("./model1_balence_best")  # ← 수정
model.to(device)
model.eval()
...
plt.tight_layout()
plt.savefig('model1_balence_results.png', dpi=150)
plt.show()
print("시각화 저장 완료: model1_balence_results.png")
