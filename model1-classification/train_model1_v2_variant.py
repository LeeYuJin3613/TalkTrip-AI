...
# 1. 데이터 로드
# ──────────────────────────────────────────
with open(r"C:\dev\TalkTrip\TripTalk_ai_model\dataset\test-add-aihub.json", 'r', encoding='utf-8') as f:
...
# ──────────────────────────────────────────
# 5. 모델
# dropout 0.4로 높임 (과적합 방지)
# ──────────────────────────────────────────
model = AutoModelForSequenceClassification.from_pretrained(
    "klue/roberta-base",
    num_labels=2,
    hidden_dropout_prob=0.4,           # 0.3 → 0.4
    attention_probs_dropout_prob=0.4   # 0.3 → 0.4
)
...
# lr 5e-6으로 낮춤 (과적합 방지)
# ──────────────────────────────────────────
optimizer = torch.optim.AdamW(model.parameters(), lr=5e-6)  # 1e-5 → 5e-6
...
class EarlyStopping:
    ...
    def __call__(self, val_loss):
        ...
        elif val_loss > self.best_score - self.min_delta:  # loss는 낮을수록 좋음
            self.counter += 1
...
# ──────────────────────────────────────────
# 8. 학습 (최대 100 에폭)
# 저장 기준: Val Loss (낮을수록 저장)
# ──────────────────────────────────────────
SAVE_DIR = "./model1_v5_dropout04_lr5e6"  # 폴더명 변경

train_losses  = []
val_losses    = []
val_accs      = []
best_val_loss = float("inf")  # val_acc → val_loss 기준
...
plt.tight_layout()
plt.savefig('model1_v4_results.png', dpi=150)
plt.show()
print("시각화 저장 완료: model1_v4_results.png")
