"""
Stage 4 + 5 + 6: 정규화 + 이벤트 조립 + 일자별 일정표 생성 (개선판 v4)

v3 → v4 주요 개선:
1. Subsumption 대폭 강화 (초당순두부 → 초당할머니순두부 완전 흡수)
2. Time이 없는 이벤트에 기본 시간대(아침/점심/오후/저녁) 자동 부여
3. Destination / start_date / duration 안정성 유지

사용법:
    python stage456_improved_v4.py --pred result.json --output summary.json --verbose
"""
...
# v4: time 없는 이벤트에 자동으로 부여할 기본 시간대
...
def format_human_readable(summary):
    lines = ["\n" + "="*60, f"여행 일정표 [{summary['chat_id']}] (v4)", "="*60]
...
if __name__ == '__main__':
    ...
    print(f"v4 개선판 실행 → 입력: {args.pred}")
