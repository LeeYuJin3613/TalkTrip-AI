"""
Stage 4 + 5 + 6: 정규화 + 이벤트 조립 + 일자별 일정표 생성 (개선판 v3)

주요 개선 (v2 → v3):
1. Destination scoring 완전 재설계 → sub-location 강력 배제 + 초반 confirmed 도시 우선
2. start_date 정규화 대폭 강화 → "이번 주말", "토요일" 등을 실제 날짜로 변환
3. Duration 추출 안정화
4. CITY_KEYWORDS / SUB_LOC_KEYWORDS 대폭 확장

사용법:
    python stage456_improved_v3.py --pred result.json --output summary.json --verbose
"""
...
CITY_KEYWORDS = ['시', '도', '제주', '서울', '부산', '강릉', '경주', '여수', '속초', '춘천',
                 '포항', '전주', '목포', '안동', '통영', '거제', '순천', '창원', '김해']

SUB_LOC_KEYWORDS = ['역', '터미널', '공항', '근처', '시장', '카페', '카페거리', '거리', '해변',
...
def is_subsumed_by(prev_text, cur_texts, threshold=0.78):
    for cur in cur_texts:
        if prev_text == cur or prev_text in cur:
            return True
        if levenshtein_ratio(prev_text, cur) >= threshold:
...
def normalize_date(text, base_date=DEFAULT_BASE_DATE):
    ...
    if m:
        ...
        if which == '다음': days_until += 7
        elif days_until == 0: days_until = 7
        target = base_date + timedelta(days=days_until)
        return target.strftime('%Y-%m-%d')
    return text
...
# ============================================================
# Stage 5: assemble_events (v2와 동일, lookback 동적)
# ============================================================
...
def assemble_events(messages, lookback=10):
    ...
    for i, msg in enumerate(messages):
        ...
        filtered_ents = []
        for ent in ents:
            if ent['type'] == 'LOC' and span_idx >= 0 and is_lodging_context_loc(span_idx, in_span_msgs, ent['text']):
                continue
...
# ============================================================
# v3 핵심: extract_meta (Destination scoring 재설계)
# ============================================================
    ...
    for p in proposals:
        ...
        for e in p['entities']:
            if e['type'] == 'DATE':
                ...
                if converted != e['text']:  # 실제 날짜로 변환됨
...
def build_schedule(proposals, total_days, destination=None, lodging=None, main_transport=None):
    ...
    for i in range(len(confirmed_sorted)-1):
        ...
        if any(CATEGORY_MAP.get(e['type']) in ('PLACE', 'LODGING') for e in prev['entities']): continue
        prev_act_food = [e for e in prev['entities'] if CATEGORY_MAP.get(e['type']) in ('ACTIVITY', 'FOOD')]
        if not prev_act_food or not all(e['text'] in GENERIC_ACTS_FOR_MERGE or len(e['text']) <= 2 for e in prev_act_food): continue
        nxt = None
        for j in range(i+1, len(confirmed_sorted)):
            cand = confirmed_sorted[j]
            if cand['idx'] - prev['idx'] > 3 or cand.get('day_hint') != prev.get('day_hint'): break
    ...
    for p in proposals:
        if p['idx'] in merged_prev_idx: p['status'] = 'absorbed'
    for p in proposals:
    ...
    for hint, group in sorted(day_groups.items(), key=lambda x: (isinstance(x[0], int), x[0])):
        ...
        for p in group:
            ...
            memo = ', '.join([f"{e['text']} 먹기" for e,_ in food_ents] + [e['text'] for e,_ in activity_ents]) or None
            if is_multi:
                for ent, cat in valid_ents:
                    days_data[day].append({'time': p['time'], 'location': ent['text'], 'category': cat, 'source_text': p['msg']['text']})
                continue
            if place_ents:
                ...
                for ent, cat in valid_ents:
                    ...
                    days_data[day].append({'time': p['time'], 'location': loc_text, 'category': cat, 'source_text': p['msg']['text']})
        inferred_day = day + 1
...
def format_human_readable(summary):
    lines = ["\n" + "="*60, f"여행 일정표 [{summary['chat_id']}] (v3)", "="*60]
