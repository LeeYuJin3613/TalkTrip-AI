"""
Stage 4 + 5 + 6: 정규화 + 이벤트 조립 + 일자별 일정표 생성 (개선판 v4.1)

v4 → v4.1: 키워드 대폭 확장
- CITY_KEYWORDS: 주요 여행 도시/지역 대폭 추가
- SUB_LOC_KEYWORDS: 실제 대화에서 자주 나오는 sub-location 추가

사용법:
    python stage456_improved_v4.1.py --pred result.json --output summary.json --verbose
"""
...
# ============================================================
# assemble_events, absorb_general_by_specific, extract_meta (v4.1)
# ============================================================
...
def get_total_days(duration_str):          # ← 이 함수가 빠져있어서 오류 발생
...
def process(pred_results, verbose=False):
    ...
    for chat_id, chat_msgs in chats:
        ...
        total_days = get_total_days(meta['duration'])   # ← 여기서 호출
...
def format_human_readable(summary):
    lines = ["\n" + "="*60, f"여행 일정표 [{summary['chat_id']}] (v4.1)", "="*60]
...
if __name__ == '__main__':
    ...
    print(f"v4.1 키워드 확장판 실행 → 입력: {args.pred}")
