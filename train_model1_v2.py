...
# 1. 데이터 로드
# ──────────────────────────────────────────
with open(r"C:\dev\TalkTrip\TripTalk_ai_model\dataset\test.json", 'r', encoding='utf-8') as f:
...
loss_fn = CrossEntropyLoss()  # 클래스 가중치 없음
model.to(device)
print(f"\n사용 디바이스: {device}")
...
for epoch in range(100):
    ...
    if val_acc > best_val_acc:
        ...
        model.save_pretrained("./model1_v2_window3")
        tokenizer.save_pretrained("./model1_v2_window3")
...
# 9. 테스트
# ──────────────────────────────────────────
model = AutoModelForSequenceClassification.from_pretrained("./model1_v2_window3")
model.to(device)
model.eval()
...
plt.tight_layout()
plt.savefig('model1_v2_window3_results.png', dpi=150)
plt.show()
print("시각화 저장 완료: model1_v2_window3_results.png")
